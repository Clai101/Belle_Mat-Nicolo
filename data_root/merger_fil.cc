void merger_fil() { // имя функции = имя файла
	...
	TH1F *hist = new TH1F("hist", "", 100, -1, 1);
	
	for (auto iter {names.begin()}; iter != names.end(); ++iter) {
		TFile *input = new TFile((*iter).c_str(), "read");
		TTree *tree = (TTree*)input->Get("h1");
		
		TH1F *temp = new TH1F("temp", "", 100, -1, 1);
		//tree->Draw("en - ecm >> temp", "p < 0.1 && en > 8 && chu == 3 && abs(ml - 2.28646) < 0.015 && abs(mach - 2.28646) < 0.015 && ntr == 1");
		tree->Draw("en - ecm >> temp", "p < 0.08 && en > 8 && chu == 5 && abs(ml - 2.28646) < 0.015 && abs(mach - 1.86484) < 0.015 && ntr == 0");
		//tree->Draw(" >> temp", "p < 0.08 && en > 8 && chu == 5 && abs(ml - 2.28646) < 0.015 && abs(mach - 1.86484) < 0.015 && ntr == 0");
		//tree->Draw("en - ecm >> temp", "p < 0.08 && en > 8 && chu == 6 && abs(ml - 2.28646) < 0.015 && abs(mach - 1.86966) < 0.015 && ntr == 0");
		//tree->Draw("en >> temp", "chu == 5"); && chu == 7
